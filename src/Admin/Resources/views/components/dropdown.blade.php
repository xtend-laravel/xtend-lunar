...
<div class="relative inline-block text-left" x-data="{ menuOpen: false }" @click.outside="menuOpen = false">
    <div>
        @if ($minimal)
            <button @click.prevent="menuOpen = !menuOpen" type="button"
    ...
    <div class="{{ $position == 'right' ? 'left-0' : 'right-0' }} absolute z-50 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
        role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1" x-cloak
        x-show="menuOpen" x-transition:enter="ease-out duration-100" x-transition:enter-start="opacity-0 scale-95"
